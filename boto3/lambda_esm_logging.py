...
def update_event_source_mappings (lambda_client, event_source_mappings, desired_log_level):
    def exec (esm):
        try:
            return lambda_client.update_event_source_mapping (
                ...
                LoggingConfig = {
                    'SystemLogLevel': desired_log_level
                }
            )
    ...
    if (desired_log_level not in valid_log_levels):
        raise ValueError (F"Invalid desired_log_level '{desired_log_level}'. Must be one of: {', '.join (sorted (valid_log_levels))}")
