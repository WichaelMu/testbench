...
PROGRAM_NAME = 'substructures-with-struct'
...
def get_recursively_relationship (relationship, current):
    ...
    result += seq (relationship) \
        ...
        .map (lambda x: get_recursively_relationship (x['curriculum_structure_relationship'], current)) \
        .to_list ()
...
def main ():
    ...
    response = dsc.get_all_academic_items ('substructures')

    substructures_with_struct = None

    if (dsc.has_generated (PROGRAM_NAME, 'substructures-with-struct') and False):
        substructures_with_struct = dsc.load_generated (PROGRAM_NAME, 'substructures-with-struct')
        dbg.dinfo (F'Loaded from {dsc.get_generated_path (PROGRAM_NAME, "substructures-with-struct")}')

    else:
        substructures_with_struct = pseq (response) \
            ...
            .map (lambda x: seq (get_what_we_need (x['nested_curriculum_structure']['curriculum_structure_container'], {
                ...
                .map (lambda k:
                {
                    'substructure-code':     k['current']['code'],
                    'substructure-name':     k['current']['name'],
                    'substructure-nickname': k['current']['nickname'],
        ...
        fq_path = dsc.save_generated (PROGRAM_NAME, 'substructures-with-struct', substructures_with_struct)

        dbg.dinfo (F'Saved. {fq_path}')

    output_path = dsc.get_generated_path (PROGRAM_NAME, 'iridescent', extension = '.xlsx')
    dsc.to_xlsx (
        substructures_with_struct,
        [ 'substructure-code', 'substructure-name', 'substructure-nickname', 'structure-category', 'structure-code', 'structure-name', 'structure-nickname' ],
        output_path
    )
